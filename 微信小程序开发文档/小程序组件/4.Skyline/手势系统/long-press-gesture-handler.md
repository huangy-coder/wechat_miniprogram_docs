...
|      | eventhandler 回调参数说明state手势状态absoluteX相对于全局的 X 坐标absoluteY相对于全局的 Y 坐标translationX相对于初始触摸点的 X 轴偏移量translationY相对于初始触摸点的 Y 轴偏移量velocityX手指离开屏幕时的横向速度（pixel per second)velocityY手指离开屏幕时的纵向速度（pixel per second) |                |        |      |                            |
