...
> 官方文档：[wx.getTransitCardInfo(Object args)](https://developers.weixin.qq.com/miniprogram/dev/api/device/nfc/wx.getTransitCardInfo.html)
> 所属分类：[设备](../设备目录.md)
> 导航路径：设备 / NFC 读写 / wx.getTransitCardInfo
> 整理日期：2026-06-01
...
仅 Android 可用。查询指定交通卡的详细信息（卡号、余额、交易记录等）
...
```javascript
const { result, errorMsg, cardInfo } = await wx.getTransitCardInfo({
  issuerID: 'changsha',
  fields: ['cardNo', 'balance', 'transactionRecords'],
});
```
